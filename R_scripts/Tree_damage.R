...
Dam<-Dam[complete.cases(Dam[,c(4,11,12)]),]
Dam<-subset(Dam,Prop_dam<0.6)
Dam<-subset(Dam,Study!="Guariguata et al 2009")
...
M1<-lmer(qlogis(Prop_dam)~Vol2*Method+Vol_sq*Method+(1|Study),Dam,na.action="na.fail")
M2<-lmer(qlogis(Prop_dam)~Vol2+Vol_sq+(1|Study),Dam,na.action="na.fail")
M3<-lmer(qlogis(Prop_dam)~Vol2+(1|Study),Dam,na.action="na.fail")
M4<-lmer(qlogis(Prop_dam)~Vol_log*Method+Vol_log2*Method+(1|Study),Dam,na.action="na.fail")
M5<-lmer(qlogis(Prop_dam)~Vol_log*Method+(1|Study),Dam,na.action="na.fail")
M0<-lmer(qlogis(Prop_dam)~1+(1|Study),Dam,na.action="na.fail")
AICc(M1,M2,M3,M0)
summary(M1)
All_mods<-list(M1,M2,M3,M4,M5,M0)

#diagnostic plots

#model averaging
ms1<-mod.sel(object=All_mods,rank="AICc",fit=T,trace=T,subset=dc(Vol2,Vol_sq))
ms1$r2<-c(r.squaredGLMM(M5)[1],r.squaredGLMM(M4)[1],r.squaredGLMM(M3)[1],r.squaredGLMM(M0)[1],r.squaredGLMM(M2)[1],r.squaredGLMM(M1)[1])
...
Damage_pred<-data.frame(Vol2=c(seq(10,107,1),seq(5,104,1)),Method=c(rep("Conventional",98),rep("RIL",100)))
